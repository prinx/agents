...
For each completed user-facing feature, validate applicable accessibility and responsive behavior, then write the simplest evidence-based local test handoff: a local URL when a server was started, otherwise exact commands and short steps. Never final-PASS without that usable path.
