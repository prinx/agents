...
- **Skill routing is automatic**: the orchestrator scans available skills on every handoff and tells the subagent which to load. The user should never need to request a skill explicitly. Route UI work to `frontend-design` (or `ui-ux`), test work to `unit-testing`/`feature-testing`/`e2e-testing`, review to `quality-review`, unclear scope to `requirements-gathering`, architecture to `solution-design`, tickets to `task-breakdown`, Git to `branch-safely`, deploy to `deploy`/`deploy-vercel`, docs to `documentation`, production to `monitoring`. Do not skip routing for small changes.
