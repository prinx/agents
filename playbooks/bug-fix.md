...
9. Quality runs all three test levels, verifies the regression tests exist and pass, and confirms the original bug scenario is fixed. It then inspects the diff for correctness, security, and scope. It classifies findings as blocking or advisory, determines the repository-backed local test path, writes `local-test.md`, and records explicit QA and review outcomes. It reports `BLOCKED` or `PASS_WITH_NOTES` if the path cannot be determined or if regression tests are missing. **Quality is mandatory — never skip.**
10. Orchestrator updates state, backlog, and memory. **Orchestrator must confirm `qa-report.md` and `review.md` exist before declaring work done.** Logs the completed fix in `state.md` with: fix description, key decisions, known issues, dependencies, and QA outcome. At completed user-facing work, it gives the human exact local commands and URL from `local-test.md` and does not present a blocked or noted path as fully complete. Deployment still requires an explicit human request and passing gates.
